# This is a tweeked version of https://github.com/thisisG/MPU6050-I2C-Python-Class
# The DMP update freqency is set at 50 Hz. To change it,
#     you need to edit the last entry of line 736 of MPUConstants.py
# To run on the Pico at 100 Hz, you need to overclock above 125Mhz. 200 is good.
# Note that we are also running with no ISR and that I2C is running at 600000Hz

import math
import time
from . import usmbus as smbus
...
class MPU6050:
    ...
    __dev_id = 0
    __bus = None
    __GyroDmpFs = 32767/2000 # This number needs to change depending on gyro scale

    def __init__(self, bus=0, sda=0, scl=1, address=C.MPU6050_DEFAULT_ADDRESS):
        self.__dev_id = address
        # Connect to num 1 SMBus
        self.__bus = smbus.SMBus(bus, scl, sda, freq=600000) #400000 default
    ...
    def get_rotation(self):
        ...
        gx = -to_short(ctypes.c_int16(raw_data[0] << 8 | raw_data[1]).value)
        gy = to_short(ctypes.c_int16(raw_data[2] << 8 | raw_data[3]).value)
        gz = to_short(ctypes.c_int16(raw_data[4] << 8 | raw_data[5]).value)
        return gx/self.__GyroDmpFs, gy/self.__GyroDmpFs, gz/self.__GyroDmpFs
...
def mpu_read_loop():
    ...
    # Initialize the DMP and enable it.
    mpu.dmp_initialize()
    mpu.set_DMP_enabled(True)
    # Print initial status (this print happens on Core 1).
    print("MPU int status:", hex(mpu.get_int_status()))
    packet_size = mpu.DMP_get_FIFO_packet_size()
    print("Packet size:", packet_size)
    FIFO_count = mpu.get_FIFO_count()
    print("Initial FIFO count:", FIFO_count)

    FIFO_buffer = [0] * 64  # Pre-allocate a buffer

    while True:
        FIFO_count = mpu.get_FIFO_count()
        mpu_int_status = mpu.get_int_status()

        # Reset the FIFO if an overflow is detected.
        ...
        # If new data is available:
            ...
            # In your code you adjust roll by +10 and use -gx as rolldot.
            with data_lock:
                sensor_data = (roll + 10, -gx)
                data_ready = True
        # Use a short delay to keep the FIFO drained.
        time.sleep_us(10)
